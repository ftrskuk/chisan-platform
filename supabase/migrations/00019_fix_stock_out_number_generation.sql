-- Fix: 1) Extract only 3-digit sequence from reason field
--      2) Store only SO number in reason field (not concatenated with user reason)
...
BEGIN
  ...
  SELECT COALESCE(
    MAX(
      CAST(
        SUBSTRING(reason FROM LENGTH(today_prefix) + 1 FOR 3) AS INTEGER
      )
    ),
    0
  ) INTO last_seq
